...
function bindEvents(container) {
  ...
  container.querySelectorAll('.acc-checkbox').forEach(cb => {
    cb.onchange = (e) => {
      const id = e.target.dataset.id;
      if (e.target.checked) analysisState.excludedAccountIds = analysisState.excludedAccountIds.filter(aid => aid !== id);
      else if (!analysisState.excludedAccountIds.includes(id)) analysisState.excludedAccountIds.push(id);
      saveState();
      refresh();
    };
  });
