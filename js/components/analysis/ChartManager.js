...
export function renderBSBalanceChart(accounts, excludedIds = []) {
  ...
  // 表示上の「純資産（バランス調整用）」を計算
  const visibleNetWorth = visibleAccounts.reduce((sum, a) => sum + Number(a.balance), 0);

  const positiveAccounts = visibleAccounts.filter(a => Number(a.balance) > 0).sort((a,b) => b.balance - a.balance);
  const negativeAccounts = visibleAccounts.filter(a => Number(a.balance) < 0).sort((a,b) => a.balance - b.balance);
  ...
  if (visibleNetWorth > 0) datasets.push({ label: '純資産', data: [0, visibleNetWorth], backgroundColor: '#10b981', stack: 'bs' });
  ...
  if (visibleNetWorth < 0) datasets.push({ label: '純資産(マイナス)', data: [Math.abs(visibleNetWorth), 0], backgroundColor: '#f59e0b', stack: 'bs' });
